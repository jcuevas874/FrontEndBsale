const host = "http://192.168.3.8:4000/"
  ...
  console.log(products)
      if (products.results.length > 0) {
           products.results.forEach( product => {
             ...
             <img height="210px" src="${url_image ? url_image : 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/300px-No_image_available.svg.png'}" class="card-img-top" alt="${name}">
             ${discount !== 0 ? `<span class="product-discount-label btn-danger bg-gradient">${discount}%</span>`: ''}
             </div>
             <div class="card-body d-flex flex-column justify-content-center">
               <h5 class="card-title text-center">${name}</h5>
               <div class="d-flex flex-row justify-content-center">
               ${discount !== 0 ? `<span class="card-text text-center text-secondary" style="text-decoration: line-through;">${price} $ - </span>`: ''}
               <span class="card-text text-center">${discount !== 0 ? price - (price * (discount ) / 100) : price} $</span>
               </div>
